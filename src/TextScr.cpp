		...
		case 1: //PARSE
			...
			while (bExit == FALSE)
			{
				if (gTS.data[gTS.p_read] == '<')
				{
					...
					else if (IS_COMMAND('B','U','Y'))
					{
						gTS.buy_cost = GetTextScriptNo(gTS.p_read + 4);
						gTS.buy_notenough = GetTextScriptNo(gTS.p_read + 9);
						gTS.buy_decline = GetTextScriptNo(gTS.p_read + 14);
						gTS.p_read += 18;
						gTS.mode = 8;
						PlaySoundObject(5, 1);
						gTS.wait = 0;
		...
		case 8: //BUY
			...
			else
			{
				...
				if (gKeyTrg & gKeyOk)
				{
					...
					{
						//No
						JumpTextScript(gTS.buy_decline);
					}
					else
					{
						if (gMC.rings >= gTS.buy_cost)
						{
							//Yes
							gTS.mode = 1;
							gTS.wait_beam = 0;
							gMC.rings -= gTS.buy_cost;
						}
						else
						{
							//Not enough rings
							JumpTextScript(gTS.buy_notenough);
						}
					}
				}
				//Yes
