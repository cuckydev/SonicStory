...
int DamageBoss(int bos, int srcX, int srcY, int damage)
{
	int bos_;
	...
	for (bos = 0; bos < BOSS_MAX; ++bos)
	{
		...
		for (bul = 0; bul < BULLET_MAX; ++bul)
		{
			...
			if (bHit)
			{
				...
				if (gBoss[bos].bits & NPC_SHOOTABLE)
				{
					DamageBoss(bos, gBul[bul].x, gBul[bul].y, gBul[bul].damage);
					if (--gBul[bul].life < 1)
						gBul[bul].cond = 0;
				}
