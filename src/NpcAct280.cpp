...
void ActNpc282(NPCHAR *npc)
{
	...
	switch (npc->act_no)
	{
		...
		case 20:
			...
			if (!gMC.status.inAir && gMC.y < npc->y - 0x800 && gMC.x > npc->x - 0x3000 && gMC.x < npc->x + 0x3000)
			{
				npc->tgt_y = 0x12000;
				npc->ani_no = 2;
			}
			...
			if (gMC.status.wallLeft && gMC.x < npc->x - npc->hit.back && gMC.x > npc->x - npc->hit.back - 0x1000 && gMC.y + gMC.yRadius > npc->y - npc->hit.top && gMC.y - gMC.yRadius < npc->y + npc->hit.bottom)
			{
				npc->bits &= ~NPC_SOLID_HARD;
				npc->ani_no = 1;
			}
			else if (gMC.status.wallRight && gMC.x > npc->x + npc->hit.back && gMC.x < npc->x + npc->hit.back + 0x1000 && gMC.y + gMC.yRadius > npc->y - npc->hit.top && gMC.y - gMC.yRadius < npc->y + npc->hit.bottom)
			{
				npc->bits &= ~NPC_SOLID_HARD;
				npc->ani_no = 1;
			}
			else if (gMC.status.wallUp && gMC.y < npc->y - npc->hit.top && gMC.y > npc->y - npc->hit.top - 0x1000 && gMC.x + gMC.xRadius > npc->x - npc->hit.back && gMC.x - gMC.xRadius < npc->x + npc->hit.front)
			{
				npc->bits &= ~NPC_SOLID_HARD;
				npc->ani_no = 1;
			}
			else if (!gMC.status.inAir && gMC.y > npc->y + npc->hit.bottom - 0x800 && gMC.y < npc->y + npc->hit.bottom + 0x1800 && gMC.x + gMC.xRadius > npc->x - npc->hit.back - 0x800 && gMC.x - gMC.xRadius < npc->x + npc->hit.front + 0x800)
			{
				npc->bits &= ~NPC_SOLID_HARD;
				npc->ani_no = 1;
			}
